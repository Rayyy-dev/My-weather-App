...
import { faSearch, faTint, faWind, faCompass } from '@fortawesome/free-solid-svg-icons';
...
function App() {
  ...
  return (
    <div className={`weather-dashboard ${theme}`}>
            ...
            <div className="forecast-grid">
              ...
              <div className="forecast-temps">
                {forecast.slice(0, 5).map((day, index) => (
                  <span key={index}>{day.day.avgtemp_c}°C</span>
                ))}
              </div>
              <div className="forecast-icons">
                {forecast.slice(0, 5).map((day, index) => (
                  <img key={index} src={day.day.condition.icon} alt="Weather icon" className="forecast-icon" />
                ))}
              </div>
              <div className="forecast-conditions">
                {forecast.slice(0, 5).map((day, index) => (
                  <span key={index}>{day.day.condition.text}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      {activeTab === 'weather' && (
        <div className="weather-details-tab">
