...
import { faSearch, faMapMarkerAlt, faTint, faWind, faThermometerHalf, faSun, faMoon, faCalendarAlt, faStar } from '@fortawesome/free-solid-svg-icons';
...
function App() {
  ...
  return (
    <div className={`weather-dashboard ${theme}`}>
      ...
      {activeTab === 'hourly' && (
        <div className="hourly-forecast" style={{border: '2px solid red', padding: '10px', margin: '10px 0'}}>
          <h3>Hourly Forecast</h3>
          {hourlyForecast.length > 0 ? (
            hourlyForecast.map((hour, index) => (
              <div key={index} className="hourly-item" style={{border: '1px solid blue', margin: '5px', padding: '5px'}}>
      ...
      {activeTab === 'astronomy' && (
        <div className="astronomy-forecast" style={{border: '2px solid red', padding: '10px', margin: '10px 0'}}>
          <h3>Astronomy Information</h3>
          {weather && weather.astronomy && weather.astronomy.astro ? (
            <div className="astronomy-grid">
              <div className="astronomy-item">
                <FontAwesomeIcon icon={faSun} className="astronomy-icon" />
                <p>Sunrise: {weather.astronomy.astro.sunrise}</p>
                <p>Sunset: {weather.astronomy.astro.sunset}</p>
              </div>
              <div className="astronomy-item">
                <FontAwesomeIcon icon={faMoon} className="astronomy-icon" />
                <p>Moonrise: {weather.astronomy.astro.moonrise}</p>
                <p>Moonset: {weather.astronomy.astro.moonset}</p>
              </div>
              <div className="astronomy-item">
                <FontAwesomeIcon icon={faStar} className="astronomy-icon" />
                <p>Moon Phase: {weather.astronomy.astro.moon_phase}</p>
                <p>Moon Illumination: {weather.astronomy.astro.moon_illumination}%</p>
              </div>
            </div>
          ) : (
